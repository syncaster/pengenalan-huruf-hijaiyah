...
if menu_choice == "📖 Belajar Huruf":
    # Menandai bahwa game perlu di-reset jika pengguna pindah ke menu lain
    ...
    for index, (name, char) in enumerate(hijaiyah_items):
        ...
        with col:
            ...
            with st.container(border=True):
                # Menampilkan karakter Arab besar dan di tengah
                st.markdown(
                    f"<h1 style='text-align: center; font-size: 80px; font-family: Arial, sans-serif;'>{char}</h1>",
                    unsafe_allow_html=True)
                # Menampilkan nama latin huruf
                st.markdown(f"<p style='text-align: center;'>{name.capitalize()}</p>", unsafe_allow_html=True)
...
# =================================================================
# --- Halaman 3: Tebak Huruf Hijaiyah (DIPERBARUI) ---
# =================================================================
    ...
    def setup_new_question():
        ...
        # --- BARU: Pilih warna acak untuk latar belakang ---
    ...
    # Reset game jika pengguna baru saja beralih ke menu ini.
    # .get('game_needs_reset', True) akan bernilai True pada saat pertama kali dijalankan
    # atau jika variabelnya di-set di menu lain.
    if st.session_state.get('game_needs_reset', True):
        setup_new_question()
        st.session_state.game_needs_reset = False

    # --- BARU: Ambil warna dari session state dan tampilkan huruf dengan latar belakang berwarna ---
    card_color = st.session_state.get('card_color', '#F0F2F6')  # Gunakan warna default jika tidak ada
    st.markdown(f"""
    <div style="background-color: {card_color}; border-radius: 15px; padding: 30px; text-align: center; margin-top: 20px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
        <h1 style='font-size: 200px; font-family: "Noto Naskh Arabic", "Arial", sans-serif; margin:0; color: #333;'>{st.session_state.correct_char}</h1>
    </div>
    """, unsafe_allow_html=True)
